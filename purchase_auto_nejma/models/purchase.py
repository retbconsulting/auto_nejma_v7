...
class PurchaseOrderLine(models.Model):
    ...
    def onchange_location_id(self):
        if self._origin:
            previous_location = self._origin.location_id
            new_location = self.location_id
            state = self._origin.state
            if previous_location != new_location and previous_location and state == 'recepted':
                raise UserError("La DV ne peut plus changer les destinations pour les OT réceptionnés.")
    ...
    def generate_internal_pickings(self):
        ...
        '''for move in picking_push.move_ids_without_package:
            move.lot_ids = [(6, 0, [self.env['stock.lot'].search([('name', '=', move.purchase_line_id.vin_number)], limit=1).id])]'''
        ...
        ## Send en email to logistique
        email_values = {
            'pols': pols
        }
        mail_template = self.env.ref('purchase_auto_nejma.transitaire_new_notified')
        #mail_template.send_mail(self.id, force_send=True, raise_exception=True, email_values=email_values)
...
class PurchaseOrder(models.Model):
    ...
    attachment_importation = fields.Binary(string="Dossier d'importation")
    attachment_payment_ticket = fields.Binary(string="Ticket de paiement")
    ...
    delivered = fields.Boolean(string="Remis au transporteur", compute='_is_delivered', store=True, default=False)
    date_bad = fields.Date(string="Date BAD")
    payment_number = fields.Char(string='N° de ticket')
    payment_amount = fields.Float(string='Montant de paiement')
    ...
    def action_bad_stage(self):
        for record in self:
            if not record.attachment_bad and not record.date_bad:
                raise ValidationError("Merci de scanner et joindre le BAD et remplir la date de BAD")
            record.state = 'bad'

    def action_payment_ticket(self):
        for record in self:
            if not record.payment_number or not record.payment_amount or not record.attachment_payment_ticket:
                raise ValidationError("Merci de renseigner le N°, le montant et le ticket de paiement")
            record.state = 'payment_ticket'
            attachment = self.env['ir.attachment'].create({
                'name': 'Reçu-OT.pdf' ,
                'type': 'binary',
                'datas': self.attachment_payment_ticket,
                'mimetype': 'application/x-pdf',  # Adjust MIME type as needed
                'res_model': 'purchase.order',
                'res_id': self.id,
            })
            mail_template = self.env.ref('purchase_auto_nejma.autonejma_recepted_ticket')
            mail_template.send_mail(record.id, force_send=True, email_values={
    ...
    def action_payment_received(self):
        for record in self:
            if not record.attachment_payment:
                raise ValidationError("Merci de scanner et joindre le reçu de paiement")
    ...
    def action_closed(self):
        for record in self:
            if not record.attachment_importation:
                raise ValidationError("Merci de scanner et joindre le dossier d'importation")
            record.state = 'closed'
